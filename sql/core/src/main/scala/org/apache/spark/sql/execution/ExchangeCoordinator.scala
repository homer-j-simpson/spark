...
private[sql] class ExchangeCoordinator(
  ...
  private def doEstimationIfNecessary(): Unit = {
    if (!estimated) {
      ...
      val partitionStartIndices = estimatePartitionStartIndices(mapOutputStatistics)

      i = 0
      while (i < numExchanges) {
        val exchange = exchanges(i)
        val rdd =
          exchange.preparePostShuffleRDD(shuffleDependencies(i), Some(partitionStartIndices))
  ...
  def postShuffleRDD(exchange: Exchange): RDD[InternalRow] = {
